...
public class CSensorUnit extends CElement {
  private static final IJSONO unitConfig = cbf.getJSONOFromString("{\"loc\":[\"xpath\",\"//*[starts-with(@test-tag,'sensorunit_') and contains(@test-tag,'_listitem')]\",\"webview\"],\"name\":\"unit\"}");

  public CElements<IText> unit;

  public CSensorUnit(IConfig config) {
    super(config);
  }

  @Override
  public CSensorUnit initComponents(int... optTimeoutInSecs) {
  ...
  @Override
  public CSensorUnit collectData() {
    if (elementExists()) {
      super.collectData();
      unit.collectData();
    }
  ...
  @Override
  protected CSensorUnit createComponents() {
    super.createComponents();
    unit = new CElements<>(CText::new, UiHelper.config.createConfig(unitConfig, this));
